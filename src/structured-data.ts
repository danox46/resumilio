...
import { absoluteUrl, claimPath, localeRoot } from "./site.js";
...
export function buildProfileJsonLd(profile: ResumilioProfile, locale: Locale) {
  ...
  return {
    ...
    "@graph": [
      ...
      { ...person, subjectOf: profile.claims.map((claim) => ({ "@id": absoluteUrl(`${claimPath(claim.id, locale)}#claim`) })) },
      ...profile.claims.map((claim) => ({
        "@id": absoluteUrl(`${claimPath(claim.id, locale)}#claim`),
...
export function buildClaimJsonLd(profile: ResumilioProfile, claim: ResumilioProfile["claims"][number], locale: Locale) {
  const pageUrl = absoluteUrl(claimPath(claim.id, locale));
  const person = personEntity(profile, locale);
  ...
  const claimEntity: Record<string, unknown> = {
    ...
    citation: evidence.map((item) => ({ "@id": `${pageUrl}#${item.id}` })),
    isPartOf: { "@id": pageUrl },
  };
  ...
  return {
    "@context": "https://schema.org",
    "@graph": [
      { "@id": pageUrl, "@type": "ProfilePage", url: pageUrl, name: claim.title[locale], inLanguage: locale, mainEntity: { "@id": person["@id"] }, about: { "@id": `${pageUrl}#claim` } },
      { ...person, subjectOf: { "@id": `${pageUrl}#claim` } },
      claimEntity,
      ...
      ...evidence.map((item) => ({
        ...
        url: item.source.url ?? `${pageUrl}#${item.id}`,
        isPartOf: { "@id": pageUrl },
      })),
    ],
  };
