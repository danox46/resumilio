...
const howToPath = (locale: "en" | "es") => locale === "en" ? "/how-to/" : "/es/como-usar/";
const entries = (["en", "es"] as const).flatMap((locale) => [localeRoot(locale), brandPath(locale), howToPath(locale), ...profile.claims.map((claim) => claimPath(claim.id, locale))].map((path) => ({ path, locale })));
...
export const GET = () => {
  const urls = entries.map(({ path, locale }) => {
    ...
    const isHowTo = path === howToPath(locale);
    const alternatePath = claim ? claimPath(claim.id, alternate) : isBrand ? brandPath(alternate) : isHowTo ? howToPath(alternate) : localeRoot(alternate);
    const defaultPath = claim ? claimPath(claim.id, "en") : isBrand ? brandPath("en") : isHowTo ? howToPath("en") : localeRoot("en");
