...
const entries = (["en", "es"] as const).flatMap((locale) => [localeRoot(locale), classicPath(locale), brandPath(locale), howToPath(locale)].map((path) => ({ path, locale })));
...
export const GET = () => {
  const urls = entries.map(({ path, locale }) => {
    ...
    const alternatePath = isClassic ? classicPath(alternate) : isBrand ? brandPath(alternate) : isHowTo ? howToPath(alternate) : localeRoot(alternate);
    const defaultPath = isClassic ? classicPath("en") : isBrand ? brandPath("en") : isHowTo ? howToPath("en") : localeRoot("en");
