...
const entries = (["en", "es"] as const).flatMap((locale) => [localeRoot(locale), ...profile.claims.map((claim) => claimPath(claim.id, locale))].map((path) => ({ path, locale })));
...
export const GET = () => {
  const urls = entries.map(({ path, locale }) => {
    ...
    const alternatePath = claim ? claimPath(claim.id, alternate) : localeRoot(alternate);
    const defaultPath = claim ? claimPath(claim.id, "en") : localeRoot("en");
