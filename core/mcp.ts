...
export function createResumilioMcpServer(defaultProfilePath = "resumilio.json"): McpServer {
  ...
  server.registerTool("connection_upsert", { description: "Create or update a relationship between graph records. Career-item connections guide discovery; organization and resource connections preserve context.", inputSchema: { ...common, value: record } }, async ({ profilePath, expectedRevision, value }) => json(await upsertConnection(profilePath, value as unknown as ResumilioProfile["connections"][number], expectedRevision)));
